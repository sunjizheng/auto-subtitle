...
typedef struct VadInstT_ {
    ...
    uint16_t feature_buf[MPL];

    uint32_t input_sample_rate;     
    uint32_t output_sample_rate;       
    uint32_t max_delay_msec;        
    int16_t* buf_2;                 
    uint32_t buf_2len;              
    ...
    uint32_t msec1;
    uint32_t msec0;
    int aggressiveness_mode;        

    int16_t* buf;                   
    uint32_t buflen;                
    uint32_t buf_offset_write;      
    uint32_t buf_offset_read;       

    int32_t  buf_status;

    uint32_t base64_result;
    uint32_t little_endian_result;
} VadInstT;

//int32_t  prev_frame_status;
//uint32_t prev_start_time;       //上一帧起点和终点
